...
| **Transit Guard sibling app** (`transit-guard/`) | **Complete & verified** 2026-08-07 — second Guard-suite product, built for the hackathon pitch. Self-contained Vite 7 + React 19 + strict TS + Tailwind 4 app on **port 5174** (`npm run dev --prefix transit-guard`; launch config `transit-guard-dev`). Mobile phone-frame SPA, 5 tabs: Scan (simulated scan + intake questionnaire), Track (custody-chain timeline + add-hand-off dropdown form + one-tap RMA reverse chain), Customs (per-country doc checklists NL/CA + customs-list-vs-sales-price valuation table), Flags (5 precomputed AI flags: $186K DAP cutoff, $39K valuation variance, $43K RMA accrual, $86K in-transit count, $6.5K Dec→Jan tax timing), Audit (1,200-unit count snapshot tying to the Close Guard fleet, custody-ledger CSV export, on-device Tesseract OCR). Trilingual UI EN/ES/FR. OCR assets self-hosted in `transit-guard/public/` (worker + wasm cores + eng/spa/fra fast traineddata) so the demo works offline; CDN fallback if assets missing. `npm run build` passes; full click path verified in-browser incl. OCR → auto-attach → customs checklist 3/5→4/5. Session state only; demo "today" is Dec 30, 2026. |
| **Transit Guard pitch deck** (`deck/Transit_Guard_Pitch.pptx`) | **Complete** 2026-08-07 — 10 slides: title, problem, solution chain diagram, product tabs, AI flags ($229K exposure), ERP-complementary positioning (NetSuite/Oracle/SAP/BlackLine), TAM $28B / SAM $6.5B / SOM $48M ARR with sourced market chart, competition, roadmap + pilot ask, closing. Validated + rendered via PowerPoint COM; speaker notes on every slide. Generator script lives in the session scratchpad (regenerate by re-running it, or edit the pptx directly). |
